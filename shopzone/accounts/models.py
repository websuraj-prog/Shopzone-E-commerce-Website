...
class CartItem(BaseModel):
    ...
    sneaker_data = models.JSONField(null=True, blank=True)

    def get_product_price(self):
        if self.sneaker_data:
            return self.sneaker_data.get('price_inr', 0) * self.quantity
...
class Order(BaseModel):
    ...
    shipping_address = models.ForeignKey('home.ShippingAddress', on_delete=models.SET_NULL, null=True, blank=True)
...
class OrderItem(BaseModel):
    ...
    def __str__(self):
        ...
        if self.sneaker_data and 'name' in self.sneaker_data:
            return f"{self.sneaker_data['name']} - {self.quantity}"
        return f"{self.product.product_name if self.product else 'Unknown Product'} - {self.quantity}"
