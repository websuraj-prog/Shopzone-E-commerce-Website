...
def remove_from_wishlist(request, uid):
    ...
    return redirect(request.META.get('HTTP_REFERER') or reverse('wishlist'))


# Wishlist View
@login_required
def wishlist_view(request):
    wishlist_items = Wishlist.objects.filter(user=request.user).prefetch_related('product__product_images', 'product__color_variant', 'size_variant')
